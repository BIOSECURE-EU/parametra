...
color_scale <- function(x) {
  ...
  if(all(x == 0, na.rm = TRUE)) return(rep("#edf8fb", length(x)))
  
  # Replace NA with 0 and create base colors
  x[is.na(x)] <- 0
  colors <- rep("#edf8fb", length(x))
  non_zero <- x > 0
  
  if(any(non_zero)) {
    non_zero_x <- x[non_zero]
    scaled_values <- (non_zero_x - min(non_zero_x)) / 
                     (max(non_zero_x) - min(non_zero_x))
    # Ensure no NA values in color assignment
    color_indices <- pmax(1, pmin(100, round(scaled_values * 99) + 1))
    colors[non_zero] <- colorRampPalette(c("#8c96c6", "#810f7c"))(100)[color_indices]
  }
  
  return(colors)
}

print("Matrix header column keys:")
print(matrix_header$col_keys)
print("Disease df column names:")
print(names(disease_df))
