...
PARAMETRA contains multiple tables with different parameter types. Let's first get an overview of what's available:
...
### Example 1: Exploring Transmission Parameters

Let's examine transmission parameters for a specific disease:
...
infectious_periods <- parametra_long %>%
  filter(parameter == "Infectious period",
         pathogen %in% c("Classical Swine Fever", "Foot and Mouth Disease", 
                        "African Swine Fever", "Avian Influenza")) %>%
  filter(!is.na(value))

# Boxplot of infectious periods by pathogen
ggplot(infectious_periods, aes(x = pathogen, y = value)) +
  geom_boxplot(aes(fill = pathogen)) +
...
### Example 3: Exploring Diagnostic Test Performance
...
test_summary <- diagnostic_tests %>%
  group_by(test_type, parameter) %>%
  summarize(
...
# Visualize diagnostic test performance
test_summary %>%
  filter(n > 2, mean_value<1) %>%  # Filter tests with sufficient data points
  ggplot(aes(x = test_type, y = mean_value, color = parameter)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = min_value, ymax = max_value), width = 0.2) +
  coord_flip() +
  labs(title = "Diagnostic Test Performance",
       x = "Diagnostic Test",
       y = "Value (0-1)",
       color = "Parameter") +
  theme_minimal()
...
### Example 4: Analyzing Data Availability Over Time

```{r}
# Count parameters by year and type
params_by_year <- parametra_long %>%
  filter(!is.na(year)) %>%
  count(year, parameter_type)

# Plot trend of parameter types over time
ggplot(params_by_year, aes(x = year, y = n, color = parameter_type)) +
  geom_line() +
  geom_point() +
  labs(title = "Livestock Disease Parameter Publications Over Time",
       x = "Year",
...
## PARAMETRA entries by pathogen and parameter type
